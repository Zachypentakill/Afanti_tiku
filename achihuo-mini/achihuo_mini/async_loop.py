...
from .adapter import Adapter, KILL_SIGNAL, KILL_TERM
from .models import make_queue, make_cache
from .exceptions import NotCommand, UnknownCommand
from .settings import DEFAULT_CACHE_BACKEND, DEFAULT_QUEUE_BACKEND
...
class AsyncLoop:
    ...
    async def __task_sleep(self):
        '''
        Define 'task_internal' to set an internal for acting one task
        '''
    ...
    def __uncache_current_queue_task(self, task_name, tdx, future):
        if task_name:
            # remove from current_task_table
        ...
        # task is over, remove from current_task_loop
        self.adapter.task_over(tdx)
    ...
    def add_task(self, generator_name, *args,
                 task_name=None, repeat=True, priority=0):
        '''
        Add one task to task queue if task_name is not in
        self.task_hash_table.
        If repeat is False, task will be added if task_name
        is not in self.current_task_hash_table.
        '''
    ...
    def __start_slave(self):
        """
        `master` does not run `run` method instead of only watch task queue
        """
    ...
    def start(self):
        ...
        if comd == 'start':
            pids = self.adapter.get_pids(script_filename)
            if self.__slave:
                pids['slave'].append(pid)
            else:
                pids['master'].append(pid)
            self.adapter.save_pids(pids, script_filename)
            ...
            if not opts:
                yes = input('Are you sure to clear ALL cache and queue (NO/yes) ')
    ...
    async def async_web_request(self, item, session=None, keep_alive=True, check_html=None):
        # !!! achihuo_mini does not handle cookies.
        # Cookies store at response.cookies
        ...
        for _ in range((item.max_retry or 1)):
            # logging.info('[async_web_request]: {}'.format(item.url))
            ...
            try:
                ...
                # html string is stored at response._content
